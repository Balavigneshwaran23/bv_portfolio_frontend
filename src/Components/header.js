...
import { HomeIcon, UserIcon, LightBulbIcon, BriefcaseIcon,DocumentIcon, PhoneIcon } from '@heroicons/react/24/outline'; 
export default function Header (){
    ...
    return<header className="  flex justify-between text-white px-5 py-2 ">
        <a data-aos="fade-down" data-aos-duration="2000"data-aos-once="true" href='https://github.com/BALAVIGNESHWARAN23' id="home" className='z-10 '><h2 className=" font-bold text-3xl  font-boldfont hover:text-orange-500  text-white  ">B<span className='hover:text-white text-orange-500 '>V</span></h2></a>
        <nav className='hidden md:block'>
        <ul data-aos="fade-down" data-aos-duration="2000"data-aos-once="true"   className="headermin flex fixed  z-50 font-bold text-2xl font-modernfont">
  ...
  <li className="flex justify-center items-center hover:text-gray1 duration-300"><DocumentIcon  className="h-5 w-5 mr-1 mt-1" /><a href={bala_resume}>Resume</a></li>
  <li className="flex justify-center items-center hover:text-gray1 duration-300"><PhoneIcon className="h-5 w-5 mr-1 mt-1" /><a href="#contact">Contact</a></li>
        ...
        <button onClick={()=> settogglebnt(!togglebnt)}><Bars3Icon  className=' fixed right-6 top-4 block md:hidden mobilnav1 text-white hover:text-orange-500 font-boldfont h-6 '/></button>
    </header>
}